...
$$2^{32}$$개의 정수를 $$N$$개 구간으로 나눌 때, $$2^{32}$$가 $$N$$의 배수가 아니면 $$2^{32} \pmod N$$ 만큼의 남는 영역이 생기며, 이게 편향을 만든다.\
이 영역의 크기를 계산한 것이 `threshold`이다.\
...
완벽한 double의 해상도(약 9조 개)를 모두 사용하지는 못하더라도 상당한 고해상도(약 42.9억 개)의 난수를 사용할 수 있는 것이다.
...
실수 변환 시 범용성과 규격 안전성을 위해서는 **`std::generate_canonical`**를 사용한다.\
