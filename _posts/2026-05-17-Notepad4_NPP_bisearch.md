...
유니코드 전 영역에 산재한 세부 범위들을 고속으로 탐색하기 위해, 아래와 같이 **구간(Interval) 검색용 이진 탐색 함수를 직접 구현**했었다.\
순차 탐색(Linear Search)으로 이 범위들을 직접 훑는 건 **성능 손실**이 너무 크다.
...
static constexpr int bisearch(const int ucs, const struct interval *table, int max) {
    ...
    if (it != table + max + 1 && ucs >= it->first)
        return 1;
...
static constexpr int bisearch_modern(const int ucs, std::span<const interval> table) {
...
반드시 **"첫 번째 인자는 배열에서 꺼낸 데이터 원소(Left), 두 번째 인자는 사용자가 찾으려는 목적어 키값(Right)"** 순서여야 한다.
...
우리가 찾는 값이 배열의 첫 번째 값보다도 작으면 `lower_bound`는 정상 포인터인 `std::begin()`을 반환하므로 메모리 참조가 안전하다.\
...
그렇다면 **더 깔끔한 코드**를 위해 STL를 쓰는 편이 더 낫다는 생각.\
그리고, 혹시나 있을 **나의 휴먼 에러**를 없앨 수 있다[…]
