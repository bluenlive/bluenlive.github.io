...
### 2. Xoshiro256: 초고속 비암호학적 의사난수 엔진
...
### 3. SFMT19937: SIMD 파이프라인에 최적화된 메르센 트위스터

SFMT19937은 고전적인 메르센 트위스터(MT19937)를 현대 CPU의 SIMD 명령어 구조에 맞게 재설계한 난수 엔진이다.\
...
### 4. ChaCha20: 암호학적 수준의 고품질 스트림 난수 엔진
...
그래프에서 보여주는 내용은 `rand()` 및 `MT19937`도 함께 비교한 결과이다.\
난수로서의 품질은 대동소이하며, 모두 평균은 0, 표준편차는 $$\frac{1}{\sqrt{12}} \approx 0.228675$$에 근접했다.\
특이할 점은 `Xoshiro256`과 `Xoshiro256`의 생성 속도가 C++ 표준인 `MT19937`의 3배에 달한다는 점.\
그리고, 암호학적 난수인 `ChaCha20`도 제대로 구현하면 `MT19937`보다 조금이나마 빠르게 동작한다는 점.

### 기능 비교

| 구분 | SplitMix64 | Xoshiro256 | Xoshiro256 | ChaCha20 |
| --- | --- | --- | --- | --- |
