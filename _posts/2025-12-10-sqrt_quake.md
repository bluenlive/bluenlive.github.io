...
이 알고리즘의 핵심은 **대단히 합리적인 초기값을 잡는 것**이다.\
그리고는 이후의 반복 연산을 2회(아래 코드에서는 3회)만 수행한다.
...
이 경우에도 약 1.3% 더 큰 값이 통상적으로 사용된다.
