...
아래 그림에서 $$beam1(= +z)$$을 $$beam2$$로 회전변환 할 때 $$el$$각과 $$az$$각을 적용하는 순서에 따라 결과가 다르다는 뜻.

![image](</images/2025-08-03/graph_B_Q.png>){: .align-center}

게다가, 어떤 각을 먼저 계산하든, 회전 결과 $$beam2$$를 $$beam1$$과 비교하면 둘 사이의 각은 의도한 $$el$$, $$az$$각과 다르다.\
...
위 코드는 $$az$$각 → $$el$$각 순으로 회전이동을 한 뒤에 원래 벡터와의 $$el$$, $$az$$ 각을 계산하는 코드이다.\
$$el$$각과 $$az$$각 중 하나가 0이면 원하는 결과가 도출되지만, 둘 다 0이 아닌 경우엔 $$az$$각이 다르게 계산된다.
...
이 문제를 해결하기 위해서는 먼저 계산하는 $$az$$각에 대해 발생할 수 있는 오차를 미리 고려해야 한다.\
$$az$$각과 $$el$$각을 동시에 적용한 벡터를 생성한 뒤 $$el$$각을 역으로 적용하면 된다.\
아래 코드는 이를 구현한 코드이다.
