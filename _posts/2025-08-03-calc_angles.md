...
위 코드는 az각 --> el각 순으로 회전이동을 한 뒤에 원래 벡터와의 el, az 각을 계산하는 코드이다.\
el각과 az각 중 하나가 0이면 원하는 결과가 도출되지만, 둘 다 0이 아닌 경우엔 az 각이 다르게 계산된다.
...
이 문제를 해결하기 위해서는 먼저 계산하는 az각에 대해 발생할 수 있는 오차를 미리 고려해야 한다.\
az각과 el각을 동시에 적용한 벡터를 생성한 뒤 el각을 역으로 적용하면 된다.\
아래 코드는 이를 구현한 코드이다.
