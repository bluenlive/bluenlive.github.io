...
하지만, **MCU(Micro Controller Unit)**나 **FPGA**, 혹은 **정수 SIMD 연산**을 극한으로 최적화해야 하는 환경에서는 이야기가 달라진다.

$$\pi$$를 정수 범위에서 다루는 접근방식 두 가지를 고민해봤다.

## 1. 왜 정수 원주율인가?

부동소수점 연산 장치(FPU)가 없는 저사양 MCU나, 전력 소모를 극단적으로 줄여야 하는 임베디드 환경에서 실수 연산은 사치이다.\
...
* **원리:** $$\text{Scaled\_Pi} = \text{round}(\pi \times 2^n)$$
