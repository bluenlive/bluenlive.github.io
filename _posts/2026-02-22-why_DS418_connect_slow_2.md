...
그렇다...\
NAS에서 PC로 대량의 데이터를 보내는 걸 실패한 것이다.
...
이게 **드라이버의 문제**인지, **HW 적인 오류**인지를 근본적으로 찾는 건 어려워 보인다.\
