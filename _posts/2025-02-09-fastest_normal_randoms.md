...
\end$$\
이렇게 생성된 $$ x $$와 $$ y $$는 서로 독립이고, 표준 정규분포 $$\begin
...
예컨데, IPP로 Box-Muller 알고리즘을 구현하는 방법이 있다.

그런데, [`sse_mathfun.h`](http://gruntthepeon.free.fr/ssemath/){:target="_blank"}와 같은 초고속 근사 라이브러리를 사용하는 방법도 있다.\
...
이렇게 float 범위 내에서 생성된 난수의 분포에 대해서 **평균(mean)**, **표준편차(stddev)** 외에도 **왜도(skewness)**, **첨도(kurtosis)** 등을 계산했다.\
...
동작 속도의 차이는 무려 25배 이상이었다.\
단, AVX-512는 인텔 CPU에서는 사실상 사용이 불가능하므로 AVX2를 사용해야 한다.\
이 때는 약 11배 이상의 속도 차이가 있었다.

double 범위 내에서 생성된 난수에 대해 같은 작업을 수행했을 때, 결과는 아래와 같았다.
