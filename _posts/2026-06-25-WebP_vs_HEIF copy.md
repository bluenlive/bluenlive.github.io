...
여기서부터는 품질 극강을 보여주는 **Tencent Canghai 2종**과 소프트웨어 인코더의 절대 기준점인 **x265**를 상수로 두고 GPU를 비교한다.\
...
H.264 시절의 인텔 QSV의 품질과 비교해보면 뭔가 많이 아쉽고 안타깝기까지 하다.
...
클라우드 ASIC은 구매하기도 쉽지 않을 뿐더러, 전용 SDK나 커스텀 빌드된 FFmpeg을 통해 CLI 환경에서 어렵게 제어해야 한다.\
반면 상용 GPU의 AV1은 프리미어, 다빈치 디졸브, OBS 등 우리가 사용하는 모든 미디어 프레임워크와 즉시 네이티브로 연동된다.\
...
내 작업 파이프라인의 목적과 생태계에 부합하는 장비가 곧 정답이다.
