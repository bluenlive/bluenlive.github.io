...
- CRC 같은 고오오오급 기술은 사용하지 않음[^2]

이 때 기본적으로 제공(?)해준 입력 프로그램은 참으로 불편했었다.\
한 바이트 입력 시마다 일일이 **리턴 키**[^3]를 눌러야 해서 그렇지 않아도 끝없이 펼쳐지는 입력 노가다를 더욱 힘들게 했었다.
...
[^2]: 아마 CRC가 사용됐으면 그 때의 나는 그 지점을 넘어가기 힘들었지 않았을까 싶음
[^3]: 당시 MSX의 키보드에는 현재 [Enter] 키라고 사용되던 키가 [Return] 키였음
