# Markdown Examples

## Headers

# Heading 1

## Heading 2

### Heading 3

#### Heading 4

##### Heading 5

###### Heading 6

## Emphasis

*Italic text*
**Bold text**
***Bold and italic text***
*single asterisks*
_single underscores_ (동작하지 않음, * 과 같은 기능)
**double asterisks**
__double underscores__ (동작하지 않음, ** 과 같은 기능)
~~cancelline~~

## Lists

### Unordered List

- Item 1
  - Item 1-1
  - Item 1-2
- Item 2
- Item 3

### Ordered List

1. First item
2. Second item
3. Third item

## Links

[GitHub](https://github.com){:target="_blank"} (target은 deprecated)
[Google](https://www.google.com){:target="_blank"} (target은 deprecated)
[`info.ko@switzerland.com`](mailto:info.ko@switzerland.com)

※ target은 지정할 필요 없도록 **루비 플러그**인 만들어둠
외부 링크는 자동으로 _blank를 지정하도록 해 둠

## Images

![GitHub Logo](https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png)
![GitHub Logo](https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png){: .align-center}
--> {: .align-center} 는 생략 가능. 지정하지 않으면 가운데 정렬을 자동 지정하도록 루비 스크립트 만들어둠

## Code Blocks

### Inline Code

`int a = 1;`

### Code Block

```c
int main() {
    printf("Hello, world!");
    return 0;
}
```

```cpp
constexpr float static fifth_root(float a, int iterations = 12) {
    if (a <= 0.0f) return 0.0f;
    float x = 1.0f;
    for (int i = 0; i < iterations; ++i) {
        x = (4.0f * x + a / (x * x * x * x)) / 5.0f;
    }
    return x;
}
```

```nasm
adosseg
 .model small
 .stack 100h

 .data
hello_message db 'goodbye, world', 0dh, 0ah, '$'

 .code
main proc
    mov ax, @data
    mov ds, ax
    mov ah, 9
    mov dx, offset hello_message
    int 21h

    mov ax, 4C00h
    int 21h
main endp
end main
```

```liquid
{% raw %}{% include gallery id="gallery_name" %}{% endraw %}
```

```batch
cmake -S %SRC_DIR% -B "%~dp0x64" -G "Visual Studio 17 2022" -A x64 ^
  -DZLIB_COMPAT=ON ^
  -DBUILD_SHARED_LIBS=OFF ^
  -DCMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded ^
  -DCMAKE_C_FLAGS="/arch:AVX2"
cmake --build "%~dp0x64" --config Release --target zlib-ng --parallel
```

```bash
sudo install -m 4755 -o root -D /var/packages/r8152/target/r8152/spk_su /opt/sbin/spk_su
sudo chmod a+rx /opt /opt/sbin
```

## Tables

| Header 1 | Header 2 |
| -------- | -------- |
| Cell 1   | Cell 2   |
| Cell 3   | Cell 4   |

## Blockquotes

> This is a blockquote.
> This is the second line.

## horizontal rules

---

## deprecated div style

### Header Boxes (not markdown) [deprecated]

<div style="background-color: #fedec7; padding: 10px; border: 1px dashed #fe8943; margin-bottom: 1.2em;"><p style="text-align: center; margin-bottom: 0;"><span style="font-size: 1.111em;"><b>
<span style="color: #980000;">빨강 계열 박스 그리기 테스트입니다</span>
</b></span></p></div>

<div style="background-color: #DBE8FB; padding: 10px; border: 1px dashed #79A5E4; margin-bottom: 1.2em;"><p style="text-align: center; margin-bottom: 0;"><span style="font-size: 1.111em;"><b>
<span style="color: #002596;">파랑 계열 박스 그리기 테스트입니다</span>
</b></span></p></div>

<div style="background-color: #DBE8FB; padding: 10px; border: 1px dashed #79A5E4; margin-bottom: 1.2em;"><p style="text-align: center; margin-bottom: 0;"><span style="font-size: 1.111em;">
<div markdown="1">
파랑 계열 마크다운 테스트입니다
</div>
</span></p></div>

<div style="background-color: #E7FDB5; padding: 10px; border: 1px dashed #9FD331; margin-bottom: 1.2em;"><p style="text-align: center; margin-bottom: 0;"><span style="font-size: 1.111em;"><b>
<span style="color: #659300;">녹색 계열 박스 그리기 테스트입니다</span>
</b></span></p></div>

<div style="background-color: #eeeeee; padding: 10px; border: 1px dashed #c1c1c1; margin-bottom: 1.2em;"><p style="text-align: center; margin-bottom: 0;"><span style="font-size: 1.111em;"><b>
<span style="color: #808080;">회색 계열 박스 그리기 테스트입니다</span>
</b></span></p></div>

<div style="padding: 10px; border: 1px dashed #c1c1c1; margin-bottom: 1.2em;"><p style="margin-bottom: 0;"><span style="font-size: 1.05em;"><b>
번역 문구 등에서 사용할 중립적 박스. 아무 내용 없이 박스만 추가함, 글자 크기도 약간 더 작게 했음
</b></span></p></div>

<div style="background-color: #fefeb8; padding: 10px; border: 1px dashed #f3c534; font-size: 0.95em; margin-bottom: 1.2em;" markdown="1">
**노랑 계열 박스 그리기 테스트입니다**
- line1
- line2
</div>

(파랑)
<div style="border-style: dashed; border-width: 1px; border-color: #79a5e4; background-color: #dbe8fb; padding: 10px;"><p style="text-align: center; margin-bottom: 0;"><span style="font-size: 1.111em;"><b><a href="/notepad4/Notepad4-r5332/">새 버전</a>이 나왔습니다. <a href="/notepad4/Notepad4-r5332/">새 버전</a>을 사용해주시기 바랍니다.</b></span></p></div><p><br /></p>

(녹색)
<div style="border-style: dashed; border-width: 1px; border-color: #9fd331; background-color: #e7fdb5; padding: 10px;"><p style="text-align: center; margin-bottom: 0;"><span style="font-size: 1.1em;"><b><a href="https://zockr.tistory.com/30" target="_blank">이전 블로그(ZockrWorld)</a>에서 일부 이미지가 사라져버려, 이미지 복원차 현 블로그로 옮긴 글입니다</b></span></p></div><p><br /></p>

<div style="position: relative; display: inline-block; padding: 15px 45px 15px 15px; margin: 5px 0; border: 1px solid #f8f861; border-left: 30px solid #f8f861; border-bottom-right-radius: 60px 10px; word-break: break-all; background: #ffff88; background: -moz-linear-gradient(-45deg, #ffff88 81%, #ffff88 82%, #ffff88 82%, #ffffc6 100%); background: -webkit-gradient(linear, left top, right bottom, color-stop(81%, #ffff88), color-stop(82%, #ffff88), color-stop(82%, #ffff88), color-stop(100%, #ffffc6)); background: -webkit-linear-gradient(-45deg, #ffff88 81%, #ffff88 82%, #ffff88 82%, #ffffc6 100%); background: -o-linear-gradient(-45deg, #ffff88 81%, #ffff88 82%, #ffff88 82%, #ffffc6 100%); background: -ms-linear-gradient(-45deg, #ffff88 81%, #ffff88 82%, #ffff88 82%, #ffffc6 100%); background: linear-gradient(135deg, #ffff88 81%, #ffff88 82%, #ffff88 82%, #ffffc6 100%); filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='#ffff88', endColorstr='#ffffc6', GradientType=1); margin-bottom: 1.2em;"><p style="margin-bottom: 0;"><span style="font-family: 'NanumPen', 'Noto Sans Kr', sans-serif; font-size: 1.45em; color: #555;"><b>
포스트잇 느낌 박스 그리기 테스트입니다 (글자 크기는 필요시 조절)
</b></span></p></div>

<div style="text-align: center;" markdown="1">
[Download Notepad2-mod_r5118.zip](/attachment/2024-01-17/Notepad2-mod_r5118.zip){: .btn .btn--info .btn--x-large}
<br>password꞉ <span style="color: red; font-size: 1.5em;"><b>teus.me</b></span>
</div>

<div style="background-color: #fedec7; padding: 10px; border: 1px dashed #fe8943; margin-bottom: 1.2em;"><p style="text-align: center; margin-bottom: 0;"><b><span style="color: #980000; font-size: 1.222em;">주의! 스포일러가 잔뜩 포함된 글입니다</span></b></p></div>

<div style="background-color: #fefeb8; padding: 10px; border: 1px dashed #f3c534; font-size: 0.95em; margin-bottom: 1.2em;" markdown="1">
ChatGPT 질문!!!!!
</div>

<div style="padding: 10px; border: 1px dashed #c1c1c1; margin-bottom: 1.2em;" markdown="1">
ChatGPT 답변!!!!!
</div>

### Quote Box (not markdown) [deprecated]

<div class="quoteMachine">
  <div class="theQuote">
    <blockquote><span class="quotationMark quotationMark--left"></span>
They call me&hellip;<br /> Mister Tiibs / 가운데 정렬
    <span class="quotationMark quotationMark--right"></span></blockquote>
  </div>
  <div class="quoteAuthor">
    - hello / 누가 썼는지 표기
  </div>
</div>

<div class="quoteMachine">
  <div class="theQuoteLeft">
    <blockquote><span class="quotationMark quotationMark--left"></span>
They call me&hellip;<br /> Mister Tiibs / 왼쪽 정렬
    <span class="quotationMark quotationMark--right"></span></blockquote>
  </div>
  <div class="quoteAuthor">
    - hello / 누가 썼는지 표기
  </div>
</div>

<div class="quoteMachine">
  <div class="theQuoteLeft">
    <blockquote><span class="quotationMark quotationMark--left"></span>
They call me&hellip;<br /> Mister Tiibs / 누가 썼는지 표기하지 않고 왼쪽 정렬
    <span class="quotationMark quotationMark--right"></span></blockquote>
  </div>
</div>

### Youtube Embed (not markdown) [deprecated]

<iframe width="640" height="360" src="https://www.youtube-nocookie.com/embed/DHQVsejt_bE?controls=0" frameborder="0" allowfullscreen></iframe>
<iframe width="640" height="360" src="https://www.youtube-nocookie.com/embed/oKbo5XDQU7s?controls=0&start=1320&end=1380" frameborder="0" allowfullscreen></iframe>
<iframe width="640" height="360" src="https://www.youtube-nocookie.com/embed/DHQVsejt_bE" frameborder="0" allowfullscreen></iframe>
<iframe width="640" height="360" src="https://www.youtube-nocookie.com/embed/oKbo5XDQU7s?&start=1320&end=1380" frameborder="0" allowfullscreen></iframe>

## Header Box [div tags removed]

### 색 박스

다음에 나오는 챕터를 **하나의 색 박스**에 아래와 같이 담음
쓸 수 있는 색은 gray, blue, yellow, orange, pink, green, brown, transparent

{: .bluebox-orange}
1. **소설**에서 본드가 요양소에 간 이유는 과다 흡연량을 줄이고 건강을 회복하기 위함이었는데, **영화**에서는 흡연 장면이 등장하지 않음

필요 시에는 아래와 같은 변칙도 활용 가능함
div 태그 내에 유튜브 동영상이 들어가는 경우에 활용 가능

<div class="bluebox-gray" markdown="1">
</div>

### 용도 박스

다음에 나오는 챕터를 **하나의 용도 박스**에 아래와 같이 담음
쓸 수 있는 용도는 spoiler, history, newversion, recon

{: .bluebox-spoiler}
파이썬으로 텍스트 파일을 읽다 문제가 터져서 빡치는 바람에 쓴 글.

### 특수 용도 박스

아래 내용들은 Liquid 스크립트로 사용할 것

{% include bnl_quote.html
   content="I've directed the first, the best and the biggest James Bond movies"
   author="Terence Young" %}

{% include bnl_quote.html
   content="때로는 이런 영화가 있습니다. 작품성은 뻔할 것 같은데 왠지 땡기는 영화. 안봐도 비디오이지만 그래도 보고 싶어지는 영화. 막상 보고나면 내가 이걸 왜 봤을까 ㅠㅠ 하면서도 그래도 땡기는 영화들이 있습니다."
   author="페니웨이™의 In This Film :: 괴작열전(怪作列傳)"
   author_url="https://pennyway.net/470"
   new_window=true %}

이 중 **newversion**, **recon**은 Liquid 스크립트가 아니라 메시지 헤더 내에서 아래와 같이 사용할 것

reconstructed:
  url: "https://zockr.tistory.com/30"
  link_text: "이전 블로그(ZockrWorld)"
  message: "에서 일부 이미지가 유실되어, 이미지 복원차 현 블로그에 재구성한 글입니다"

new_version_url: "/myprogram/iPress-v3.20/"

## Quote Box [div tags removed]

인용구는 아래와 같이 지정
align은 생략시 "left"이며, author는 생략 가능
content는 여러 줄이 가능하며, 각 줄은 | 로 구분함 (즉, |는 문구에 들어가지 못함)
또한, 여러 줄을 쓸 때 `1.`, `-` 등으로 시작하면 ol/ul로 변환하는데 이에 대한 대비는 없음에 유의

{% include bnl_quote.html
   align="center"
   content="I've directed the first, the best and the biggest James Bond movies"
   author="Terence Young" %}

## Download Box [div tags removed]

다운로드는 아래와 같이 기술 가능
filename이 생략되면 file에서 적절히 추출하며, description은 생략 가능함

{% include bnl_download-box.html
   file="/attachment/2025-11-28/Any2MP3_v4.45.rar"
   filename="Any2MP3_v4.45.rar"
   description="64bit only"
   password="teus.me" %}

## AI 질문/답변

질문은 아래와 같이

{: .ai-question}
영국 런던 사진으로 알고 있는데, 정확한 장소를 찾아서 알려줘

답변은 아래와 같이

{: .ai-answer}
사진 속 장소는 영국 런던의 **Red Telephone Boxes(붉은 공중전화 부스)**가 있는 거리입니다.

간단히 쓸 수도 있음
하지만, AI의 답변은 실제로는 길어지기 때문에 아래와 같이 `capture` 기능을 사용할 것
단, 내부에 각주가 들어갈 수 없음을 기억할 것

{% capture answer0 %}
사진 속의 위치는 아마도 **Covent Garden의 Broad Court**, Royal Opera House 맞은편에 있는 **빨간 공중전화 부스 다섯 대가 나란히 서 있는 거리**로 보입니다. 이곳은 런던 중심부에서 사진 찍기 좋은 명소 중 하나예요.

특히, Waymarking 정보에 따르면 이 전화 부스들이 있는 정확한 좌표는 다음과 같습니다:
{% endcapture %}
{% include bnl_ai-answer.html content=answer0 %}

## 알고리즘 기술

AI 답변과 사실상 같은 메커니즘임을 참고
.ai-answer --> .algorithm

단, 내부에 각주가 들어갈 수 없음을 기억할 것

{: .algorithm}
$$
q_1 = (a_1, b_1, c_1, d_1) \quad\text{(w, x, y, z 순서)}
$$

위와 같이 간단히 쓸 수도 있음
하지만, 알고리즘 기술은 실제로는 길기 때문에 아래와 같이 `capture` 기능을 사용할 것

{% capture algo0 %}
$$
q_1 = (a_1, b_1, c_1, d_1) \quad\text{(w, x, y, z 순서)}
$$

$$
q_2 = (a_2, b_2, c_2, d_2)
$$
{% endcapture %}
{% include bnl_algorithm.html content=algo0 %}

## Youtube Embed [div tags removed]

유튜브는는 아래와 같이 기술
id만 적으면 됨
시작시간, 종료시간 지정은 아래와 같이 초 단위로 지정

{% include video id="yL9eRIFQJGE" provider="youtube" %}
{% include video id="oKbo5XDQU7s?start=1320&end=1380" provider="youtube" %}

본문에서 사용할 때는 `{% include` 앞에 공백 없이 사용하고,
리스트 내에서 사용할 때는 앞에 리스트 마커 길이만큼 띄어쓰기를 해서 사용할 것
(`1. 어쩌고` 포맷에서는 3칸 `- 어쩌고` 포맷에서는 2칸 띄어쓴다는 뜻)

## Postit

Postit 스타일은 아래와 같이 지정
content는 여러 줄이 가능하며, 각 줄은 | 로 구분함 (즉, |는 문구에 들어가지 못함)
이 내용은 친효 스킨에서 모두 빌려옴
또한, 여러 줄을 쓸 때 `1.`, `-` 등으로 시작하면 ol/ul로 변환하는데 이에 대한 대비는 없음
다음을 사용할 것
• (bullet, U+2022) → 가장 직관적이고 깔끔
‣ (triangular bullet, U+2023) → 조금 더 강조된 느낌
▪ (small square, U+25AA) → 네모 포인트
→ (right arrow) → 진행/순서 강조
⇒ (double arrow) → 강한 강조
➤ (black arrowhead) → 리스트 포인트로 자주 사용
①, ②, ③ … (circled numbers) → 순서 강조, 깔끔한 번호 매김
1), 2), 3) → 괄호 붙이면 마크다운 리스트로 인식되지 않음

{% include bnl_postit.html content="오늘의 결론: 아프지 맙시다." %}

## 책 제목, 영화 제목 (old style)

[영화 제목]은 기존대로 유지
\<책 제목, 게임 제목\>은 꺾쇠로 감싸는데 이스케이프 시퀀스 잊지 말 것, 《...》 이 좋긴 한데 입력이 불편함
영화 제목 등에 콜론이 필요하면 --> ꞉ (U+A789)

## 책 제목, 영화 제목

각종 제목 표시는 한글맞춤법/국립국어연구원 표기법을 따르도록 수정
https://www.law.go.kr/%ED%96%89%EC%A0%95%EA%B7%9C%EC%B9%99/%ED%95%9C%EA%B8%80%EB%A7%9E%EC%B6%A4%EB%B2%95
https://www.korean.go.kr/nkview/nknews/200103/32_3.htm
아래와 같이 수정했으며, 이를 위한 PowerToy를 만들어둠
〈영화제목, 노래제목〉: U+3008(〈), U+3009(〉) --> Ctrl+Alt+,
《책제목》: U+300A(《), U+300B(》) --> Ctrl+Alt+.
작은 따옴표: U+2018(‘), U+2019(’) --> Ctrl+Alt+'
큰 따옴표: U+201C(“), U+201D(”) --> Ctrl+Alt+Shift+'
콜론: U+A789(꞉) --> Ctrl+Alt+;
이 과정에서 asterisk(\*) 표시는 U+2217(∗)로 대체 --> Ctrl+Alt+Shift+8
␣(visible space) U+2423 --> Ctrl+Alt+Space

## 문법 강조

백틱 문법을 사용할 수도 있고, 리퀴드 태그를 사용할 수도 있음.
어느 쪽을 사용하든지, 기본은 행번호가 없으며, 행번호 및 행 강조를 부여할 수 있음

```python
def hello():
    print("Hello, world!")
```

```bash
!analyze -v
```

```bat
ffmpeg -f concat -i "frames.txt" -vsync vfr "output.mp4"
```

### 행 번호 표시: 백틱 코드

{: data-linenos="true"}
```html
<head>
    <link rel="stylesheet" type="text/css" href="style.css" media="all" />
    <script src=""></script>
</head>
```

### 행 강조: 백틱 코드

{: data-hl-lines="2-3"}
```html
<head>
    <link rel="stylesheet" type="text/css" href="style.css" media="all" />
    <script src=""></script>
</head>
```

※ 물론 `{: data-linenos="true" data-hl-lines="2-3"}` 처럼 둘을 섞어 사용할 수 있음
※ 이 IAL 형식 태그는 백틱 코드 바로 앞 또는 바로 뒤에 위치하면 됨

### 행 번호 표시: 리퀴드 코드

{% highlight ruby linenos=table %}
def hello_world
  puts 'Hello, world!'
  puts 'This line will be highlighted'
  puts 'Another normal line'
  puts 'This one highlighted too'
end
{% endhighlight %}

### 행 강조: 리퀴드 코드
{: data-hl-lines="2..3,5"}
{% highlight ruby linenos=table %}
def hello_world
  puts 'Hello, world!'
  puts 'This line will be highlighted'
  puts 'Another normal line'
  puts 'This one highlighted too'
end
{% endhighlight %}

※ 리퀴드 코드에서 이 IAL 형식 태그는 반드시 앞에 등장해야 함
※ 공백 또는 콤마로 구분하며, 대시(-) 또는 점2개(..)으로 범위 설정 가능

### 문법 강조된 코드 내용 복사

참조 사이트: https://luckylooky2.github.io/github-%EB%B8%94%EB%A1%9C%EA%B7%B8/copy-to-clipboard/
이 기능은 highlight 태그로 만든 코드에는 사용할 수 없고, ```(코드)```  형식의 fenced code block 에서만 동작함

{% include code-header.html %}

```jsx
export default function App() {
  return <div>hello</div>;
}
```

### 문법 강조 전체 목록 (Jekyll 기준)

https://simpleit.rocks/ruby/jekyll/what-are-the-supported-language-highlighters-in-jekyll/

### 문법 강조시 이런 거 안 됨

  ```python
  def hello():
      print("Hello, world!")
  ```

```python linenos
def hello():
    print("Hello, world!")
```

## notice

여러분 이거 참고하세요
{: .notice}

## mathjax

아래 사이트에서 쉽게 작성 가능
https://latexeditor.lagrida.com/

(inline math)
$$\frac{2S}{3} = 2a + 2b + 2c + d + e$$

(block math, inline에서도 물론 사용 가능)
$$\begin{align}
\frac{2S}{3} & = 2a + 2b + 2c + d + e\\
\frac{S}{3} & = a + b + c + 2d
\end{align}$$

(align* 도 가능함. 단, 이것도 서로 매치되어야 함)
$$\begin{align*}
i+m-1 & \le n-l-1\\
i+m & \le n-l\\
i+m & \lt n-l+1\\
i & \lt (n-m-l+1)
\end{align*}$$

$$Linear = \left\{ \begin{array}{cl}
\frac {S}{12.92} & : \ 0 \leq S \leq 0.04045 \\
\left ( \frac {S + 0.055}{1.055} \right ) ^{2.4} & : \ 0.04045 \lt S \leq 1
\end{array} \right.$$

**$$S = 3a + 3b + 3c + 3d + e$$**

## ol 항목 번호 붙이기

1. 이렇게 하면
2. 번호가 알아서 붙음
3. 특정 번호(4)부터 시작하려면 아래 처럼 할 것
{:start="4"}

## 글 접기/펼치기

<details>
<summary>👉 더보기 / 접기</summary>
<div markdown="1">
  이 부분은 기본적으로 접혀 있다가,  
  클릭하면 펼쳐져서 보입니다.  

  - 리스트도 가능  
  - 이미지, 코드 블록도 가능  
  - 마크다운 문법도 잘 적용돼요
</div></details>

## 시리즈 글

글의 헤더 영역에 다음의 둘을 기록할 것
특히, series 내용이 동일해야 시리즈 글로 인식됨
---
series: "Synology_NAS_Speed_Issue"
series_title: "시놀로지 NAS 접속 속도 이슈"
---

## 스타일 추가

single asterisk 스타일 뒤에 다음 둘을 추가하면 살짝 작을 글꼴로 적용됨
또한, --> ==> >>> 를 본문에 쓰면 ⟶ ⟹ ➤ 로 변환함

콜론을 3개 또는 4개로 감싸면 3개는 부가 설명 스타일, 4개는 주석 스타일로 적용됨
기존의 * 이나 _ 로 감싸는 것과 유사한 문법 적용되며, 글 내에 링크도 적용 가능함
:::※ 문장 문장:::
::::// 주석 주석::::

## 슬라이드 쇼

아래와 같은 형식을 기본으로 함
이미지 파일명은 콤마(,)로 구분하고
이미지에 텍스트를 추가할 때는 | 로 구분해서 적으며, 텍스트는 생략 가능

본문에서 사용할 때는 `{% include` 앞에 공백 없이 사용하고,
리스트 내에서 사용할 때는 앞에 리스트 마커 길이만큼 띄어쓰기를 해서 사용할 것
(`1. 어쩌고` 포맷에서는 3칸 `- 어쩌고` 포맷에서는 2칸 띄어쓴다는 뜻)
이건 비디오 플러그인과 100% 동일함

{% include slider.html images="/img1.jpg | 설명, /img2.jpg" %}

※ 예제
{% include bnl_slider.html images="/images/2026-03-02b/ltk1_okl_s64.jpg | hello, /images/2026-03-02b/ltk2_okl_s64.jpg | world, /images/2026-03-02b/ltk3_okl_s64.jpg" %}

자동재생을 원하면 interval를 지정할 것
이 값을 지정하지 않거나 0이면 수동 슬라이드
{% include slider.html
   interval=5000
   images="/img1.jpg | 설명, /img2.jpg"
%}

모든 옵션은 아래와 같음
type은... 기본은 'loop'이고 'slide', 'fade'도 가능 (loop는 slide가 loop 되는 것)
speed는 슬라이드 애니메이션 속도(ms)
interval은 자동 슬라이드 재생 간격(ms)
{% include slider.html
   type="fade"
   speed=1000
   interval=2000
   images="/img1.jpg | 설명1, /img2.jpg | 설명2"
%}

## 기타 등등

ex) --> e.g.,
'23년 --> 2023년, 우리말 표기법에 없는 표기임
전각 공백: (　) U+3000, 반각 공백: ( ) U+0020
여러 줄 표시는 공백 2개 보다는 \ 가 나은 듯
구라제거기의 버전 번호는 _includes/head/custom.html 에 저장됨
[^1] 형식으로 각주를 사용할 수 있음. 본문에는 [^1], 글의 끝에는 [^1]: 각주내용을 기록
